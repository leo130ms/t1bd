...
export async function executarECompararConsulta(url: string, submission: string, key: string){
    ...
    try{
        ...
        const html2 = await page2.evaluate(() => document.documentElement.outerHTML);
        const json2 = html2json(html2);
        const validResponseKey = json2.child[0].child[2].child[1].child[0].child[0].child[0].text;
...
export async function executarECompararConsultaPRO(url: string, submission: string, key: string, chkCols: number, chkOrder: number){
    ...
    try{
        ...
        if( (validResponseSubmission === 'true') && (validResponseKey === 'true') && ((responseSubmission === responseKey) ||
          ( (chkCols == 0) && (chkOrder == 1) && (responseKeyJSON._rows.toString() == responseSubmissionJSON._rows.toString()) ) || 
          ( (chkCols == 1) && (sameNames) && (chkOrder == 0) && containedLines == responseKeyJSON._rows.length) ||
          ( (chkCols == 0) && (chkOrder == 0) && (containedLines == responseKeyJSON._rows.length ) ) ) ) {
            console.log('1');
        } else {
            console.log('0');            
        }
